...
Make `selectLine` and `selectParentSyntax` create undirectional ranges.

Change the default key binding for `toggleBlockComment` to Ctrl-A on MacOS, to avoid interfering with keyboard layouts that use Alt-A for a special character.

### New features

The new `canUndoSelection` and `canRedoSelection` functions can be used to query whether a selection can be undone/redone in a given state.

## 6.10.4 (2026-06-23)

### Bug fixes

Respect selection ranges' `undirectional` flag when expanding them.
