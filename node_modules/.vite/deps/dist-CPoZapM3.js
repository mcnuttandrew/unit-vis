import { n as keyName, r as StyleModule, t as base } from "./w3c-keyname-CXWP5R0r.js";
//#region node_modules/@codemirror/text/dist/index.js
...
function mapSet(setA, setB, before, mkSet = false) {
	...
	for (let posA = 0, posB = 0;;) if (a.ins == -1) {
		posA += a.len;
		a.next();
		...
		for (;;) if (b.ins >= 0 && posB > posA && posB + b.len < end) {
			len += b.ins;
			posB += b.len;
			b.next();
...
function composeSets(setA, setB, mkSet = false) {
	...
	let a = new SectionIter(setA), b = new SectionIter(setB);
	for (let open = false;;) if (a.done && b.done) return insert ? new ChangeSet(sections, insert) : new ChangeDesc(sections);
	else if (a.ins == 0) {
...
var EditorSelection = class EditorSelection {
	...
	*/
	static range(anchor, head, goalColumn) {
		...
		return head < anchor ? new SelectionRange(head, anchor, goal | 24) : new SelectionRange(anchor, head, goal | (head > anchor ? 4 : 0));
	}
};
...
function dynamicFacetSlot(addresses, facet, providers) {
	...
	return {
		...
		update(state, tr) {
			if (!dynamic.some((dynAddr) => ensureAddr(state, dynAddr) & 1)) return 0;
			let value = get(state);
...
//#endregion
//#region node_modules/@codemirror/rangeset/dist/index.js
/**
...
function scrollRectIntoView(dom, rect, side, x, y, xMargin, yMargin, ltr) {
	...
	for (let cur = dom; cur;) if (cur.nodeType == 1) {
	...
	sync(track) {
		if (this.dirty & 2) {
			...
			for (let child of this.children) {
				...
				if (child.dom.parentNode == parent) while (next && next != child.dom) next = rm(next);
...
function replaceRange(parent, fromI, fromOff, toI, toOff, insert, breakAtStart, openStart, openEnd) {
	...
	while (fromI < toI && insert.length) if (children[toI - 1].become(insert[insert.length - 1])) {
		toI--;
		insert.pop();
...
var CompositionView = class extends WidgetView {
	...
	destroy() {
		var _a;
		super.destroy();
		(_a = this.widget.topView) === null || _a === void 0 || _a.destroy();
	}
	get isEditable() {
		return true;
	}
...
function inlineSiblingRect(view, side) {
	...
	while (parent && index >= 0) if (side < 0 ? index > 0 : index < parent.children.length) {
...
function attrsEq(a, b) {
	...
	for (let key of keysA) if (keysB.indexOf(key) == -1 || a[key] !== b[key]) return false;
	return true;
}
function updateAttrs(dom, prev, attrs) {
	...
	if (attrs) {
		for (let name in attrs) if (!(prev && prev[name] == attrs[name])) dom.setAttribute(name, attrs[name]);
	}
}
/**
...
var scrollTo = /*@__PURE__*/ StateEffect.define({ map: (range, changes) => range.map(changes) });
var centerOn = /*@__PURE__*/ StateEffect.define({ map: (range, changes) => range.map(changes) });
...
*/
var PluginField = class PluginField {
	/**
...
function computeOrder(line, direction) {
	...
	for (let i = 0, sI = 0, context = 0, ch, br, type; i < len; i++) if (br = Brackets[ch = line.charCodeAt(i)]) {
		if (br < 0) {
	...
	for (let i = 0; i < len; i++) if (types[i] == 256) {
		let end = i + 1;
		while (end < len && types[end] == 256) end++;
		let beforeL = (i ? types[i - 1] : outerType) == 1;
		let replace = beforeL == ((end < len ? types[end] : outerType) == 1) ? beforeL ? 1 : 2 : outerType;
		for (let j = i; j < end; j++) types[j] = replace;
		i = end - 1;
	}
	let order = [];
	if (outerType == 1) for (let i = 0; i < len;) {
...
var DocView = class extends ContentView {
	...
	update(update) {
		...
		if (this.dirty == 0 && changedRanges.length == 0) return false;
		else {
	...
	for (let i = 0; i < len; i++) {
		...
		for (let j = 0; j < rects.length; j++) {
			...
			if (rect.left - 1 <= x && rect.right + 1 >= x && dy < closestDY) {
				...
				if (browser.chrome || browser.gecko) {
					if (textRange(node, i).getBoundingClientRect().left == rect.right) after = !right;
				}
				if (dy <= 0) return {
					node,
					offset: i + (after ? 1 : 0)
...
var InputState = class {
	...
	ensureHandlers(view) {
		...
		for (let set of handlers) for (let type in set.handlers) if (this.registeredEvents.indexOf(type) < 0 && type != "scroll") {
			this.registeredEvents.push(type);
			view.contentDOM.addEventListener(type, (event) => {
...
var HeightOracle = class {
	...
	heightForGap(from, to) {
		...
		if (this.lineWrapping) lines += Math.ceil((to - from - lines * this.lineLength * .5) / this.lineLength);
...
var HeightMapBranch = class extends HeightMap {
	...
	lineAt(value, type, doc, top, offset) {
		...
		if (left) return base.join(this.right.lineAt(rightOffset, subQuery, doc, rightTop, rightOffset));
...
function visiblePixelRange(dom, paddingTop) {
	...
	for (let parent = dom.parentNode; parent && parent != body;) if (parent.nodeType == 1) {
		let elt = parent;
...
var ViewState = class {
	...
	getViewport(bias, scrollTarget) {
		...
		let viewport = new Viewport(map.lineAt(visibleTop - marginTop * 1e3, QueryType.ByHeight, doc, 0, 0).from, map.lineAt(visibleBottom + (1 - marginTop) * 1e3, QueryType.ByHeight, doc, 0, 0).to);
		if (scrollTarget) {
			...
			if (head < viewport.from || head > viewport.to) {
				...
				viewport = new Viewport(map.lineAt(topPos - 500, QueryType.ByHeight, doc, 0, 0).from, map.lineAt(topPos + viewHeight + 500, QueryType.ByHeight, doc, 0, 0).to);
			}
		}
		return viewport;
	}
	...
	ensureLineGaps(current) {
		...
		for (let line of this.viewportLines) {
			...
			for (let { from, to } of outside) if (to - from > 1e3) gaps.push(find(current, (gap) => gap.from >= line.from && gap.to <= line.to && Math.abs(gap.from - from) < 1e3 && Math.abs(gap.to - to) < 1e3) || new LineGap(from, to, this.gapSize(line, from, to, structure)));
		}
		return gaps;
	}
...
function applyDOMChange(view, start, end, typeOver) {
	...
	if (start > -1) {
		...
		if (diff) {
			if (browser.chrome && view.inputState.lastKeyCode == 13 && diff.toB == diff.from + 2 && reader.text.slice(diff.from, diff.toB) == "￿￿") diff.toB--;
			change = {
				from: from + diff.from,
				to: from + diff.toA,
		...
		if (change.from >= sel.from && change.to <= sel.to && change.to - change.from >= (sel.to - sel.from) / 3 && (!newSel || newSel.main.empty && newSel.main.from == change.from + change.insert.length) && view.inputState.composing < 0) {
			...
			if (startState.selection.ranges.length > 1 && view.inputState.composing >= 0 && change.to <= sel.to && change.to >= sel.to - 10) {
				...
				tr = startState.changeByRange((range) => {
					...
					if (range.to - range.from != size || view.state.sliceDoc(from, to) != replaced || compositionRange && range.to >= compositionRange.from && range.from <= compositionRange.to) return { range };
	...
	update(transactions) {
		...
		try {
			...
			for (let tr of transactions) {
				...
				for (let e of tr.effects) if (e.is(scrollTo)) scrollTarget = new ScrollTarget(e.value);
...
var handleKeyEvents = /*@__PURE__*/ EditorView.domEventHandlers({ keydown(event, view) {
...
function getBase(view) {
	...
	return {
		left: (view.textDirection == Direction.LTR ? rect.left : rect.right - view.scrollDOM.clientWidth) - view.scrollDOM.scrollLeft,
...
function measureRange(view, range) {
	...
	function piece(left, top, right, bottom) {
		return new Piece(left - base.left, top - base.top - .01, right - left, bottom - top + .01, "cm-selectionBackground");
	}
...
function iterMatches(doc, re, from, to, f) {
	re.lastIndex = 0;
	for (let cursor = doc.iterRange(from, to), pos = from, m; !cursor.next().done; pos += cursor.value.length) if (!cursor.lineBreak) while (m = re.exec(cursor.value)) f(pos + m.index, pos + m.index + m[0].length, m);
}
...
//#endregion
export { CharCategory as A, Text as B, Range as C, Annotation as D, RangeValue as E, Prec as F, findColumn as G, codePointSize as H, StateEffect as I, fromCodePoint as K, StateField as L, EditorState as M, Facet as N, ChangeDesc as O, MapMode as P, Transaction as R, scrollPastEnd as S, RangeSetBuilder as T, countColumn as U, codePointAt as V, findClusterBreak as W, highlightSpecialChars as _, Direction as a, placeholder as b, PluginField as c, ViewUpdate as d, WidgetType as f, highlightActiveLine as g, dropCursor as h, Decoration as i, EditorSelection as j, ChangeSet as k, PluginFieldProvider as l, drawSelection as m, BlockInfo as n, EditorView as o, __test as p, BlockType as r, MatchDecorator as s, BidiSpan as t, ViewPlugin as u, keymap as v, RangeSet as w, runScopeHandlers as x, logException as y, combineConfig as z };
